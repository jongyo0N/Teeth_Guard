...
class Guardian:
    ...
    def create(guardian_data: Dict[str, Any]) -> int:
        ...
        with DatabaseManager().get_db_connection() as conn:
            ...
            # nickname 중복 확인 (nickname이 제공된 경우에만)
            if validated_data.get('nickname'):
                cursor.execute("SELECT guardian_id FROM guardians WHERE nickname = %s", 
            ...
            # 보호자 생성 (nickname 포함)
            insert_query = """
                INSERT INTO guardians (name, nickname, phone, email, password_hash, experience_level) 
                VALUES (%s, %s, %s, %s, %s, %s)
            """
            values = (
                validated_data['name'],
                validated_data.get('nickname'),  # nickname은 선택사항
    ...
    def get_by_id(guardian_id: int) -> Optional[Dict[str, Any]]:
        """ID로 보호자 조회 (nickname 포함)"""
        with DatabaseManager().get_db_connection() as conn:
            cursor = conn.cursor(dictionary=True)
            
            query = """
                SELECT guardian_id, name, nickname, phone, email, experience_level, 
    ...
    def update(guardian_id: int, update_data: Dict[str, Any]) -> bool:
        ...
        with DatabaseManager().get_db_connection() as conn:
            ...
            # nickname 중복 확인 (자신 제외, nickname이 제공된 경우에만)
            if validated_data.get('nickname'):
                cursor.execute(
                    "SELECT guardian_id FROM guardians WHERE nickname = %s AND guardian_id != %s", 
                    (validated_data['nickname'], guardian_id)
                )
                if cursor.fetchone():
                    raise ValidationError("이미 사용 중인 닉네임입니다.")
            ...
            update_query = """
                UPDATE guardians 
                SET name = %s, nickname = %s, phone = %s, email = %s, experience_level = %s, updated_at = NOW()
                WHERE guardian_id = %s
            """
            values = (
                validated_data['name'],
    ...
    @staticmethod
    def check_nickname_availability(nickname: str) -> bool:
        """닉네임 사용 가능 여부 확인 (새로 추가)"""
        if not nickname or len(nickname.strip()) < 2:
            return False
        
        with DatabaseManager().get_db_connection() as conn:
            cursor = conn.cursor()
            cursor.execute("SELECT COUNT(*) FROM guardians WHERE nickname = %s", (nickname.strip(),))
            count = cursor.fetchone()[0]
            return count == 0
        ...
        with DatabaseManager().get_db_connection() as conn:
            ...
            if result:
                ...
                if result['birth_date']:
                    ...
                    birth_date = datetime.datetime.fromisoformat(result['birth_date']).date()
                    today = date.today()
