...
class DentalDiagnosisSupervisor:
    ...
    def _assess_periodontitis_node(self, state: DentalDiagnosisState) -> DentalDiagnosisState:
        ...
        # total_score 계산 추가
        diagnosis = state['diagnosis_results']
        caries_ratio = diagnosis.get('caries_ratio', 0)
        calculus_ratio = diagnosis.get('calculus_ratio', 0)
        # 비율을 0~100으로 변환
        caries_score = caries_ratio * 100
        calculus_score = calculus_ratio * 100
        stage_score = 20 * stage
        avg_score = (caries_score + calculus_score + stage_score) / 3
        total_score = 100 - avg_score
        # 0~100 범위로 보정
    ...
    async def diagnose(self, guardian_id: int, pet_id: int, image_path: str) -> Dict[str, Any]:
        ...
        result = {
            ...
            "total_score": final_state.get('total_score', None)  # total_score 추가
        }
...
async def main():
    ...
    try:
        ...
        print(f"    total_score:{diagnosis.get("total_score",0)}")
